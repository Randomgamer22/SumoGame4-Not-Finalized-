class Room {
    ...
    play() {
        ...
        ellipse(displayWidth/2, displayHeight/2, 852);

        
        player.updatePlayerPosition();
        player.getPlayerPosition();

        collideObjects(this.player1, this.player2);
        ...
        if(this.player1.y <= displayHeight/2 - 450){
            room.updateGameState(2)
            room.gameState = 2;
            push();
            textAlign(CENTER);
            textSize(40);
            text(this.allPlayers[0].name + " won the game with " + this.allPlayers[0].taps, displayWidth/2, displayHeight/2);
            pop();
        }
        else if(this.player2.y >= displayHeight/2 + 450) {
            room.updateGameState(2)
            room.gameState = 2;
            push();
            textAlign(CENTER);
            textSize(40);
            text(this.allPlayers[1].name + " won the game with " + this.allPlayers[1].taps, displayWidth/2, displayHeight/2);
            pop();
        }
